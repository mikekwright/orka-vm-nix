...
let
  cfg = config.services.orkaVm.opencode;
  homeDirectory = config.home.homeDirectory;
  resolvedOpencodePackage =
    if opencodePackage != null then
      opencodePackage
    else if pkgs ? opencode then
      pkgs.opencode
    else
      throw "services.orkaVm.opencode.package must be set when no OpenCode package is available.";
in
{
  options.services.orkaVm.opencode = {
    enable = lib.mkEnableOption "OpenCode web service and local MCP tooling";

    package = lib.mkOption {
      type = lib.types.package;
      default = resolvedOpencodePackage;
      description = "The OpenCode package to run for the local web service.";
    };

    computerControlPackage = lib.mkOption {
      type = lib.types.package;
      default = pkgs.callPackage ./pkgs/computer-control-mcp { };
      description = "The computer-control MCP package exposed to OpenCode.";
    };

    enableDirectAuth = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Enable OpenCode's own HTTP basic auth using ~/.opencode-password when present.";
    };

    port = lib.mkOption {
      type = lib.types.port;
      default = 9081;
      description = "Loopback port for the OpenCode web service.";
    };
  };
  ...
  config = lib.mkIf cfg.enable (
    let
      ...
      opencodeUrl = "http://${opencodeHost}:${toString cfg.port}";

      mcpServers = {
        "computer-control" = {
          package = cfg.computerControlPackage;
          config = {
            type = "local";
            command = [ "${cfg.computerControlPackage}/bin/computer-control-mcp" ];
            enabled = true;
          };
        };
      };
      ...
      opencodeWebStart = pkgs.writeShellScript "opencode-web-start" ''
        ...
        exec ${cfg.package}/bin/opencode web --hostname ${opencodeHost} --port ${toString cfg.port}
      '';
      ...
      assertions = [
        {
          ...
          message = "services.orkaVm.opencode requires home.homeDirectory to be set.";
        }
      ];
